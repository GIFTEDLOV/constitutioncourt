...
**RULED AND FINALIZED ON-CHAIN.** `NON_COMPLIANT` → `REJECTED`, citing `ART-4.3`.
...
| **Ruling tx** | `0x868c28dc9c2686a8a9ccffa7f05e9137e88082da11f968bcfd7e9e6c75460a16` — **`FINALIZED`**, execution `FINISHED_WITH_RETURN`, consensus `AGREE`, 0 rounds |
| Outcome | `NON_COMPLIANT` |
| Final status | `REJECTED` |
| Violated rules | `ART-4.3` |
| `ruled_at` | `2026-08-04T15:19:21Z` |
| Validator vote | 3 `AGREE`, 1 `DETERMINISTIC_VIOLATION`, 1 `TIMEOUT` → `AGREE`, single round |

![Ruling detail showing violated rule ART-4.3 supermajority approval threshold, the consensus-scope note, and citations anchored to commit-pinned evidence URLs](docs/assets/screenshots/case-002-ruling-detail-desktop.png)
...
340,000 / 520,000 ≈ **65.4%**, short of the two-thirds bar of 66.7%. The verdict
matched the expectation recorded **before** the case was filed.

### It took two attempts, and that is part of the record

| | |
| --- | --- |
| **2026-08-02** | A ruling executed and the contract read `RULED` for about a day — but it never finalized. A duplicate submission 8 seconds earlier held the per-contract queue slot, and Bradbury eventually **canceled both** ruling transactions (`0xe22b0ff6…` and `0x42b6a679…`, both status 8, terminal). The case reverted to `OPEN` and the claim was withdrawn from this README. |
| **2026-08-04** | Write-side availability recovered. **One** `rule()` call against the same contract — no redeploy, no new case — reached `FINALIZED`. |

The two rulings were produced by **different validator sets two days apart** and
reached the **same outcome and the same rule id**. Only one validator,
`0x4f36CB2C…`, sat in both sets. That is a reproducibility result the project
did not set out to run and would not have got from a single successful attempt.

It is also why this repository distinguishes `accepted` from `finalized`
everywhere: the first ruling was accepted and was still lost. **Only `FINALIZED`
is durable.**
...
| Verdict reproducible across independent validator sets | **Yes** — two sets, two days apart, identical verdict |
...
| `case-002-non-compliant-two-thirds` | `NON_COMPLIANT` → `REJECTED` | **ruled and finalized live — matches the expectation** |
...
- **Settlement is slow, and currently unreliable.** Bradbury serialises
  ...
  accepted but not finalized can still be canceled by the network** — that
  happened to Case 002's first ruling, which is why this project distinguishes
  `accepted` from `finalized` everywhere rather than treating the first as the
  second. The case was re-ruled and finalized on 2026-08-04.
